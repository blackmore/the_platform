 ...
 * @version  $Id: ElementGrid.php,v 1.2 2007/04/01 07:28:14 justinpatrin Exp $
 */
...
class HTML_QuickForm_ElementGrid extends HTML_QuickForm_element {
    ...
    // modified by S.Arnold
    function setClass($name) {
         $this->updateAttributes(array('class' => $name));
     }
    ...
    function toHtml()
    {
        ...
        foreach (array_keys($this->_rows) as $key) {
            ...
            ++$row;
				...
				// erste Zeile per span auffüllen, falls es mehrere gibt
				if( $tbody->getRowCount() > 1 ) $tbody->setCellAttributes(0,1, "colspan=".($tbody->getColCount()-1));
